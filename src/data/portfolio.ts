...
export const projects: LocalizedProject[] = [
  ...
  },
  {
    ...
    problem: {
      id: "Bisnis F&B menghadapi kebocoran biaya laten: resep & Bill of Materials (BOM) tidak terhubung ke kasir sehingga HPP riil per menu tidak pernah diketahui pasti, stok opname manual di spreadsheet sering selisih tanpa audit trail, porsi berlebih (over-portioning) dan bahan kedaluwarsa menggerus margin kotor, serta sistem kasir berhenti total saat koneksi internet mati.",
      en: "F&B businesses suffer from severe margin leakages: disconnected recipe Bill of Materials (BOM) make true dish-level COGS invisible, manual spreadsheet stock-takes generate untraceable variances, kitchen over-portioning and ingredient spoilage silently erode profits, and cloud-only POS terminals crash entirely during internet drops.",
    },
    solutions: {
      id: [
        "Resep Bertingkat & Kalkulasi HPP per Menu (BOM Murni & Semi-Prep): Mendukung resep menu langsung dari bahan mentah maupun item olahan setengah jadi (prep item seperti sambal/saus batch). Tiap porsi terjual di POS otomatis mendebit HPP dan memotong stok bahan baku pada base UOM terkecil (gram/ml) secara real-time.",
        "Stok Opname Sistematis & Rekonsiliasi VIRTUAL: Manajemen persediaan berbasis mutasi berpasangan (stock_move). Stok opname fisik langsung membandingkan stok sistem vs fisik, mencatat selisih ke akun penyesuaian (Adjustment) tanpa merusak integritas buku besar.",
        "Cost Guard & Deteksi Anomali Penerimaan Barang (GRN): Menolak otomatis input harga beli bahan yang menyimpang ekstrem (>9x Moving Average) untuk mencegah salah ketik satuan (misal harga per-dus diinput per-pcs) yang merusak kalkulasi HPP.",
        "POS Offline-First (SQLite + Capacitor): Seluruh transaksi kasir, kalkulasi pajak daerah PBJT/PB1, dan cetak struk berjalan penuh secara lokal di device saat offline, lalu sinkronisasi otomatis satu arah (idempotent) saat koneksi kembali.",
        "Tiga Buku Besar Universal (Three Ledgers Contract): Memisahkan batas arsitektur menjadi stock ledger, value ledger, dan document (ADR-0005) — modul POS tidak perlu tahu logika akuntansi, modul inventori fokus pada kuantitas fisik murni.",
        "6 Permukaan Aplikasi Terintegrasi: POS Kasir, Backoffice Finansial, Kitchen Display System (KDS), Menu QR Self-Order pelanggan, Website brand, dan Super-Admin — dibangun dengan React 19, Hono, Drizzle, dan PostgreSQL RLS.",
      ],
      en: [
        "Multi-Tier Recipe BOM & Real-Time Dish COGS: Supports both direct raw-ingredient recipes and semi-finished prep batches (e.g. prepared sauces). POS checkout sales automatically calculate dish-level COGS and deduct inventory in granular base UOMs (grams/ml) in real-time.",
        "Systematic Stock-Take & Virtual Ledger Reconciliation: Inventory movements track paired stock_move entries. Physical stock-takes record variances directly into dedicated Adjustment accounts with zero loss of transaction history.",
        "Cost Guard & Inbound Outlier Detection: Automated validation rejecting goods receipt prices deviating >9x from Moving Average, eliminating costly unit-of-measure typo errors (e.g. entering case prices as piece prices).",
        "Offline-First Register Engine (SQLite + Capacitor): Counter checkout, local PBJT/PB1 hospitality tax calculations, and receipt printing function seamlessly during internet outages with idempotent server sync upon reconnect.",
        "Three Universal Ledgers Architecture: Decouples domain logic into stock ledger, value ledger, and documents (ADR-0005) — POS never depends directly on general ledger accounting internals.",
        "6 Unified Operational Surfaces: Cashier POS, Financial Backoffice, Kitchen Display System (KDS), Customer Tabletop QR Self-Order, Brand Portal, and Super-Admin — powered by React 19, Hono, Drizzle, and Postgres RLS.",
      ],
    },
